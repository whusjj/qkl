...
import torch
import sys
sys.path.append('/root/autodl-tmp/qkl_master')
...
class Main:
    ...
    def load_model(self):
        ...
        if not os.path.exists(self.pickle_path):
            ...
            # Train the model
        graph_data.shuffle()
        train_data = graph_data[:int(0.64*len(graph_data))]
        valid_data = graph_data[int(0.64*len(graph_data)) + 1 : int(0.8*len(graph_data))]
        test_data = graph_data[int(0.8*len(graph_data)) + 1:]
        # 存储 训练数据
        if not os.path.exists(self.data_args.train_pickle_path):
            with open(self.data_args.train_pickle_path, 'wb') as handle:
                pickle.dump(train_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open(self.data_args.train_pickle_path, 'rb') as handle:
                train_data = pickle.load(handle)
        # 测试集
        if not os.path.exists(self.data_args.test_pickle_path):
            with open(self.data_args.test_pickle_path, 'wb') as handle:
                pickle.dump(test_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open(self.data_args.test_pickle_path, 'rb') as handle:
                test_data = pickle.load(handle)
        # 有效集
        if not os.path.exists(self.data_args.valid_pickle_path):
            with open(self.data_args.valid_pickle_path, 'wb') as handle:
                pickle.dump(valid_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open(self.data_args.valid_pickle_path, 'rb') as handle:
...
if __name__ == "__main__":
    ...
    data_args.train_pickle_path = f"processed/train/_vocab_size_{get_vocab_size(tokenizaer_args.token_vocab)}"
    data_args.test_pickle_path = f"processed/test/_vocab_size_{get_vocab_size(tokenizaer_args.token_vocab)}"
    data_args.valid_pickle_path = f"processed/valid/vocab_size_{get_vocab_size(tokenizaer_args.token_vocab)}"
