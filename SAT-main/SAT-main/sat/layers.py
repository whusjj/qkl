...
class Attention(gnn.MessagePassing):
    ...
    def __init__(self, embed_dim, num_heads=8, dropout=0., bias=False, epoch=2000,layers = 6,train_length=10000,test_length = 1000,val_length = 1000,batch_size = 128,
        ...
        self.train_length = math.ceil(train_length / batch_size)
        self.test_length = math.ceil(test_length / batch_size)
        self.val_length = math.ceil(val_length / batch_size)
    ...
    def forward(self,
        ...
        if self.se == 'khopgnn': # k-subgraph SAT
            ...
            # preserve pt files
        if  self._now_epoch < self.epoch:
                self.times = self.times + 1
        if  self.times == self.train_length :
            self.times = 0
            self._now_epoch = self._now_epoch + 1
        if self._now_epoch == self.epoch and self.time < self.train_length:
            if os.path.exists("/root/autodl-tmp/SAT-main/datasets/result/graph") and self.time == 0:
                print("djfuisdfgidwbwefh")
                shutil.rmtree("/root/autodl-tmp/SAT-main/datasets/result/graph/")
                os.mkdir("/root/autodl-tmp/SAT-main/datasets/result/graph/")
                os.mkdir("/root/autodl-tmp/SAT-main/datasets/result/graph/raw")
                os.mkdir("/root/autodl-tmp/SAT-main/datasets/result/graph/raw/train/")
                os.mkdir("/root/autodl-tmp/SAT-main/datasets/result/graph/raw/test/") 
                os.mkdir("/root/autodl-tmp/SAT-main/datasets/result/graph/raw/val/")
                os.mkdir("/root/autodl-tmp/SAT-main/datasets/result/graph/processed/") 
            torch.save(x_struct.cpu().detach().numpy(), f"/root/autodl-tmp/SAT-main/datasets/result/graph/raw/train/x_struct_{self.time}.pt")
        elif self._now_epoch == self.epoch and self.time < self.train_length + self.val_length:
            torch.save(x_struct.cpu().detach().numpy(), f"/root/autodl-tmp/SAT-main/datasets/result/graph/raw/val/x_struct_{self.time}.pt")
        elif self._now_epoch == self.epoch and self.time < self.train_length + self.val_length + self.test_length:
            torch.save(x_struct.cpu().detach().numpy(), f"/root/autodl-tmp/SAT-main/datasets/result/graph/raw/test/x_struct_{self.time}.pt")
...
class KHopStructureExtractor(nn.Module):
    ...
    def forward(self, x, edge_index, subgraph_edge_index, edge_attr=None,
        ...
        self.times = self.times + 1
        x_struct_sum=x_struct.sum()
        # print(x_struct.shape)
        # traceback.print_stack()
        # print(x_struct_sum)
        # torch.save(x_struct, f"/root/autodl-tmp/SAT-main/datasets/result/graph/x_struct_{self.times}.pt")
        return x_struct
...
class TransformerEncoderLayer(nn.TransformerEncoderLayer):
    ...
    """
    def __init__(self, d_model, nhead=8, dim_feedforward=512, dropout=0.1,layers = 6,
                activation="relu", batch_norm=True, pre_norm=False, epoch=2000,train_length=10000,test_length = 1000,val_length = 1000,
                gnn_type="gcn", se="gnn", k_hop=2, **kwargs):
        super().__init__(d_model, nhead, dim_feedforward, dropout, activation)

        self.self_attn = Attention(d_model, nhead, dropout=dropout,epoch=epoch,test_length = test_length,train_length=train_length,
            val_length=val_length,bias=False, gnn_type=gnn_type, se=se, k_hop=k_hop, **kwargs)
        self.batch_norm = batch_norm
