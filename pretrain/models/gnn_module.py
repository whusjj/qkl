...
class GNNModule(torch.nn.Module):
    def __init__(self, input_dim: int = 32, output_dim: int = 32, num_layers = 5, edge_dim = None,sequence = 128):
    ...
    def forward(self, graph: Graph) -> torch.Tensor:
        ...
        for gcn_layer in self.gcn:
            ...
            # pdb.set_trace()
            x = self.relu(gcn_layer(x.reshape(-1,self.input_dim), edge_index, edge_attr=edge_attr.reshape(-1,self.input_dim)))
            if self.training:
